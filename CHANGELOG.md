...
- ✨ **智能文件命名优化** - 新增智能中间截断功能
  - 支持超长文件名智能截断（最大80字符）
  - 优先使用 `title_part` 格式（如：视频标题_P1.mp4）
  - 保留标题首尾关键信息，截断中间部分
  - 智能处理重复文件命名冲突

- ✨ **Part 字段类型兼容性修复**
  - 同时支持数字类型（i64）和字符串类型（string）
  - 支持嵌套路径解析（part、page_data.part、video_info.part 等）
  - 完全向后兼容
...
```rust
// 智能中间截断示例
原标题: "Bilibili缓存转换器 - 最强桌面应用推荐完整教程 2024版"
截断后: "Bilibili缓存转换器...版_01.mp4" (保留首尾关键信息)

// 优先级
1. entry.json 中的 title + part → "视频标题_P1.mp4"
2. entry.json 中的 title → "视频标题.mp4"
3. 目录名 → "截断后的目录名.mp4"
```
