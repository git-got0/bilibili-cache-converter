...
- **智能中间截断**: 超长文件名自动截断（最大80字符），保留首尾关键信息
- **Part 字段兼容**: 同时支持数字和字符串类型的 part 字段
- **系统托盘增强**: 关闭询问、最小化托盘、退出清理进程
...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  文件扫描   │ → │ 元数据提取  │ → │  智能命名   │ → │  格式转换   │ → │  输出组织   │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
      │                  │                  │                  │                  │
  扫描 .blv/        读取 entry.json    生成友好文件名    FFmpeg 转换      保存到 result/
  .m4s/.ts 等        提取 title/part    支持截断         GPU 加速         目录结构优化
```

### 工作流程说明

1. **文件扫描**: 自动扫描指定目录，识别 Bilibili缓存文件 (.blv、.m4s、.flv、.ts)
2. **元数据提取**: 从 entry.json 读取 title、part 等信息
3. **智能命名**: 根据元数据生成友好的输出文件名（视频标题_P1.mp4）
4. **格式转换**: 使用 FFmpeg 进行音视频格式转换，支持 GPU 硬件加速
5. **目录优化**: 保持原始结构，移除冗余目录（c_开头的目录、≤3位的纯数字目录）
6. **输出组织**: 结果保存在源目录的 `/result` 子目录下
...
4. **查看进度**: 观察实时速度、剩余时间、已完成数量
5. **打开结果**: 转换完成后直接打开输出文件夹
...
download/
├── video1/
│   ├── entry.json  {"title": "教程上", "part": 1}
│   └── video.blv
└── video2/
    ├── entry.json  {"title": "教程下", "part": 2}
    └── video.blv
